...
# Init common variables, do msf login, and make a console
...
# Generates a template of an msfrpc request
    ...
    # Login to msf, return the auth_token needed for subsequent requests
    def login(self):
    ...
    def run(self, params=[], auth=True, console=True):
        ...
        if __name__ == '__main__':
            ...
            # Loop through each host running SSH login against it
            for host in hosts:
            ...
            if len(response) > 0:
                ...
                for sess_id in response:
                    print ("%-15s%s@%s" % (
                        sess_id, response[sess_id].get('username'), response[sess_id].get('target_host')))
            else:
